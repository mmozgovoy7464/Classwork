    ...
    private static ArrayList<Term> createAndAddTerms(String termString){
    	...
    	while(termString.length()>0){
    		...
    		if(indexOfMinus<indexOfPlus){
    			...
    			endOfTerm = indexOfPlus;//guarantees that either + or - comes first.
    		}
    		try{
    			...
    			//cut out the thing that was added, including the next + or -.
    			termString = termString.substring(endOfTerm+1, termString.length());
    			
    		}catch(Exception e){
    			//if the user does something that throws out an error, skip this term.
...
public static String getNoTricks(String usersInput) {
	...
	return "I cannot interpret your input";
...
//		}
//	}
//	solveQuadratic(side, a, b, c);
//	if(a == 1){
//		factor1 = factor(side, a, b, c)[0];
//		factor2 = factor(side, a, b, c)[1];
//		//print "(" + side.get(0).getVariable() + " - " + factor1 + ")(" + side.get(0).getVariable() + " - " + factor2 + ")" + " = 0";
//		//print "x = " + -factor1 + ", x = " + -factor2;
//	}
//}
}

public static double[] factor(ArrayList<Term> side, double a, double b, double c){
	double factorMax = a*c;
	double factor1 = 0;
	double factor2 = 0;
	for(double i = -factorMax; i <= factorMax; i++){
		for (double j = -factorMax; j <= factorMax; j++){
			if(i*j==factorMax && i+j==b){
				factor1 = i;
				factor2 = j;
			}
		}
	}
	double[] factors = {factor1, factor2};
	return factors;
	
}

public static double solveQuadratic(ArrayList<Term> side, double a, double b, double c){
	double d;
	d = (b*b)-(4*a*c);
	return d;
}
